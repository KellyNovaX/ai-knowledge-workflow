...
创建 GitHub Release，标签必须为 `0.3.2`，与 manifest 版本完全一致，不加 `v`。把 `dist/main.js`、`dist/manifest.json`、`dist/styles.css` 分别作为附件上传；不要只上传 ZIP。可同时附上两个 ZIP 与 SHA256SUMS.txt，方便手动安装和分享。
